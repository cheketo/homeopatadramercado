...
var MultiBox = new Class({
	...
	setContentType: function(link){
		...
		switch(str){
			...
			default:
				...
				this.type = 'iframe';
				this.contentObj.width = 600;
				this.contentObj.height = 250;
	...
	getOpenClosePos: function(el){
		...
		if(el.getFirst()){
			...
			//alert(el.getFirst().getCoordinates().width+' menos '+ this.container.getStyle('border').toInt()*2);
			//alert('Top: '+el.getFirst().getCoordinates().top);
			//alert(el);
			//alert(this.container.getStyle('border').toInt());
			if(w < 0){w = 0}
			...
			//alert(el.getCoordinates().width+' menos '+ this.container.getStyle('border').toInt()*2);
			if(w < 0){w = 0}
	...
	open: function(el){
		...
		this.hideControls.bind(this).delay(500);
		...
		if(!this.opened){
			...
			//alert('Top:'+document.body.offsetHeight/2+' menos '+this.options.initialHeight/2+' menos '+this.container.getStyle('border').toInt());
			this.containerEffects.start({
				width: this.options.initialWidth,
				height: this.options.initialHeight,
				top: (document.body.offsetHeight/2)-(this.options.initialHeight/2)-this.container.getStyle('border').toInt(),
				left: (document.body.offsetWidth/2)-(this.options.initialWidth/2)-this.container.getStyle('border').toInt(),
				opacity: [0, 1]
			});
